...
void nnet::run_sequential(input_data &data)
{
	...
	auto layer_count = (layers.size() < 0) ? 0 : signed(layers.size());
	...
	for (auto epochs = 0; epochs < EPOCHS; ++epochs)
	{
		for (auto samples = 0; samples < data.size; ++samples)
		{
			...
			layers[0]->fwd(ar_x);
			for (auto neur_it = 1; neur_it < layer_count; ++neur_it)
			{
				layers[neur_it]->fwd(layers[neur_it - 1]->get_ar_y());
			}

			/* Error */
			nnet_math<type_t>::matrix_sub(layers[layer_count - 1]->get_ar_y(), ar_t, ar_error_out);
			...
			layers[layer_count - 1]->bkwd(ar_error_out);
			layers[layer_count - 1]->set_error();
			for (auto neur_it = layer_count - 2; neur_it >= 0; --neur_it)
			{
				layers[neur_it]->bkwd(layers[neur_it + 1]->get_ar_error());
				layers[neur_it]->set_error();
			}

			/* Accumulate Error Step */
			for (auto neur_it = layer_count - 1; neur_it > 0; --neur_it)
			{
				layers[neur_it]->accm(layers[neur_it - 1]->get_ar_y());
			}
			layers[0]->accm(ar_x);
		}
	...
	layers[0]->check(x_check0);
	layers[1]->check(layers[0]->get_ar_y());
	/* If error_check0 is all 0's, training was correct for sample */
	nnet_math<type_t>::matrix_sub(layers[1]->get_ar_y(), t_check0, error_check0);
	...
	layers[0]->check(x_check1);
	layers[1]->check(layers[0]->get_ar_y());
	/* If error_check0 is all 1's, training was correct for sample */
	nnet_math<type_t>::matrix_sub(layers[1]->get_ar_y(), t_check1, error_check1);
	...
	layers[0]->check(x_check2);
	layers[1]->check(layers[0]->get_ar_y());
	/* If error_check2 is all 0's, training was correct for sample */
	nnet_math<type_t>::matrix_sub(layers[1]->get_ar_y(), t_check2, error_check2);
}
	...
	for (auto epochs = 0; epochs < EPOCHS; ++epochs)
	{
		...
		concurrency::parallel_for(0, int(accels.size()), [=, &mydata, &total_error, &error_den](const unsigned i)
		{
			...
			while (mydata.try_pop(dp))
			{
				...
				/* Error */
				nnet_math<type_t>::matrix_sub(layers[layer_count - 1]->get_ar_y(), test_t, ar_error_out);
				nnet_math<type_t>::scalar_div(ar_error_out, data.size, ar_error_out);
